...
class Res2_FFDNet(nn.Module):

    def __init__(self):
        super(Res2_FFDNet, self).__init__()
        self.num_conv_layers = 12
        self.downsampled_channels = 15
        self.num_feature_maps = 96
        self.output_features = 12
    ...
    def forward(self, x, noise_sigma):
        ...
        x_cat = x_cat.to(device)                           
        h_dncnn = self.intermediate_dncnn(x_cat)
        y_pred = my_utils.upsample(nn.functional.relu(h_dncnn+x_up))          #Downsampled residual around dncnn
        return y_pred
    
class Res3_FFDNet(nn.Module):

    def __init__(self):
        super(Res3_FFDNet, self).__init__()
        self.num_conv_layers = 12
        self.downsampled_channels = 15
        self.num_feature_maps = 96
        self.output_features = 12
    ...
    def forward(self, x, noise_sigma):
        ...
        x_cat = x_cat.to(device)                          
        h_dncnn = self.intermediate_dncnn(x_cat)
        y_pred = nn.functional.relu(my_utils.upsample(h_dncnn)+x.data)     #Added full-size residual around dncnn
        return y_pred
...
class ResidualFFDNet(nn.Module):
    ...
    def __init__(self):
        ...
        for _ in range(self.num_conv_layers - 2):
            ...
            # layers.append(nn.BatchNorm2d(self.num_feature_maps))
            # layers.append(nn.ReLU(inplace=True))
            layers.append(resBlock(in_channels=self.num_feature_maps,out_channels=self.num_feature_maps))
...
class ResidualLargeFFDNet(nn.Module):
    ...
    def __init__(self):
        super(ResidualLargeFFDNet, self).__init__()
...
class AttentionFFDNet(nn.Module):
    ...
    def forward(self, x, noise_sigma):
        ...
        h_dncnn = self.attention(h_dncnn).reshape((-1,12,h,w))
